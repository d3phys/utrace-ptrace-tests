/* Test the syscall-restart-disable should work even with x86_64 debugger and
   i386 debuggee.  Test specifically with a machine generated segfault instead
   of just artifical raise().
...
#define DEBUGEE_PATHNAME "./erestartsys-trap-debuggee"
...
# define REGISTER_ERESTARTSYS .rax
#elif defined __powerpc64__
/* FIXME:
# define REGISTER_IP .nip
# define user_regs_struct pt_regs
*/
/* FIXME: # define REGISTER_ERESTARTSYS */
#endif
...
main (int argc, char **argv)
{
  ...
  /* Its size must match `erestartsys-trap-debuggee.c'.  */
  char buf[0x100];
  size_t buf_have = 0;
  ...
  switch (child)
    ...
    case -1:
      ...
      assert (0);
    case 0:
      execl (DEBUGEE_PATHNAME, "erestartsys-trap-debuggee", "foo", NULL);
      assert (0);
    default:
      break;
    }
...
#ifdef REGISTER_ERESTARTSYS
  /* Ensure we are really inside a restart of ERESTARTSYS (-512).  */
  assert (-user_orig REGISTER_ERESTARTSYS >= 512
	  && -user_orig REGISTER_ERESTARTSYS < 512 + 64);
  // https://bugzilla.redhat.com/show_bug.cgi?id=1591459#c5
  // user_orig REGISTER_ERESTARTSYS = -EINTR;
#endif
